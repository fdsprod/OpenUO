...
namespace Client.Graphics
{
    ...
    {
        const float InvTileSize = 1f / 44f;
        ...
        private bool _normalised = false;
        ...
        {
            _normalised = useNormalisedCoordinates;
        }
        ...
        bool ICamera.GetProjectionMatrix(ref Matrix matrix, ref Vector2 drawTargetSize, ref int changeIndex)
        {
            ...
            {
                changeIndex = 1;
                ((ICamera)this).GetProjectionMatrix(out matrix, ref drawTargetSize);
                return true;
            }
        ...
        {
            float width = drawTargetSize.X * InvTileSize;
            float height = drawTargetSize.Y * InvTileSize;

            matrix = Matrix.OrthoLH(width, height, 0, 1);
        }
        ...
        void ICamera.GetViewMatrix(out Matrix matrix)
        {
            if (_dirty)
            {
                ...
                _viewMatrix = Matrix.Translation(_position) * Matrix.RotationZ(rotation);
            }
